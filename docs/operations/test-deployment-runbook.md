...
8. Run `scripts/Confirm-TestMigrationExecution.ps1` with the exact resource group, job, execution, migration-image digest, and expected last migration. The script queries the live Container Apps execution and logs, requires `Succeeded`, validates the deterministic `DLS_MIGRATION_RESULT` marker and exact image, writes a SHA-256-bound `migrationVerification` object, and only then enables `deployApplications`. Run a third `what-if`, then deploy API/worker/web. Resource tags retain the execution and evidence hash. No migration runs in API startup.
